...
<script>
    let itinerary = [
        { name: "start", type: "start", category: "Location", value: "Luxembourg", startTime: "", endTime: "", before: null },
        { name: "transit1", type: "transit", category: "Flight", value: "", startTime: "", endTime: "", before: 1 },
        { name: "end", type: "stop", category: "Location", value: "Barcelona", startTime: "", endTime: "", before: 0 }
    ];
    ...
    async function openFlightOverlay(name) {
        const transit = itinerary.find(i => i.name === name);
        const index = itinerary.findIndex(i => i.name === name);
        const from = findClosestAirport(itinerary[index - 1]?.value)
        const to = findClosestAirport(itinerary[index + 1]?.value)
        //const from = itinerary[index - 1]?.value;
        //const to = itinerary[index + 1]?.value;
        const date = itinerary[index - 1]?.startTime;
        //const date = transit.startTime?.slice(0, 10);

        if (!from || !to) return alert("Set locations and start time first");

        //console.log(`assets/php/api/fetch-flights.php?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}&date=${date}`)

        try {
            const response = await fetch(`assets/php/api/fetch-flights.php?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}&date=${date}`);
            ...
            if (!data.flights || data.flights.length === 0) {
                listContainer.innerHTML = "<div>No flights found</div>";
                return;
            }

            data.flights.forEach(flight => {
                const div = document.createElement("div");
                div.textContent = `${flight.airline}: ${flight.departure_time} → ${flight.arrival_time}`;
                div.onclick = () => {
                    transit.value = `${flight.airline} ${flight.departure_time} → ${flight.arrival_time}`;
                    closeFlightModal();
                    renderItinerary();
                };
            ...
            document.getElementById("flightModalTitle").textContent = `Available Flights (${from} → ${to})`;
    ...
    // Function to send the location and fetch the closest airport
    function findClosestAirport(location) {
        //const location = document.getElementById('location').value;

        // Send the location to PHP backend using AJAX (fetch)
        fetch('assets/php/api/get_closest_airport.php?location=' + encodeURIComponent(location))
            .then(response => response.json())
            .then(data => {
                if (data.error) {
                    document.getElementById('result').innerHTML = 'Error: ' + data.error;
                } else {
                    // Display the closest airport details
                    /*document.getElementById('result').innerHTML = `
                            Closest Airport: ${data.name} <br>
                            IATA: ${data.iata} <br>
...
<!--<script>
    let itinerary = [
        { name: "start", type: "start", category: "Location", value: "Luxembourg", startTime: "", endTime: "", before: null },
        { name: "transit1", type: "transit", category: "Flight", value: "", startTime: "", endTime: "", before: 1 },
        { name: "end", type: "stop", category: "Location", value: "Barcelona, Catalonia, Spain", startTime: "", endTime: "", before: 0 }
    ];
    ...
    function showTransportOptions(transitName, category) {
        ...
        const options = {
            Flight: ["08:00", "13:00", "19:30"],
            Train: ["07:00", "12:15", "18:45"],
            Car: ["Drive via A7 (6h)", "Drive via E25 (6.5h)", "Drive via D934 (7h)"]
        };

        const results = (options[category] || []).map((opt, idx) => {
            return category === "Car"
                ? `Route option ${idx + 1}: ${opt}`
                : `${category} from ${prev.value} to ${next.value} at ${opt} on ${date}`;
        });
        ...
        results.forEach(result => {
            ...
            div.onclick = () => selectTransport(transitName, result);
            listContainer.appendChild(div);
        });

        document.getElementById("flightModalTitle").textContent = `Available ${category}s (${prev.value} → ${next.value})`;
        document.getElementById("flightModal").style.display = "block";

        // Optional: visualize on map
        console.log(`Would display ${category} route on map from ${prev.value} to ${next.value}`);
    }

    function selectTransport(name, option) {
        const item = itinerary.find(i => i.name === name);
